#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
# Source0 file verified with key 0x575159689BEFB442 (dev@fpcomplete.com)
#
Name     : stack
Version  : 2.7.3
Release  : 17
URL      : https://github.com/commercialhaskell/stack/releases/download/v2.7.3/stack-2.7.3-linux-x86_64.tar.gz
Source0  : https://github.com/commercialhaskell/stack/releases/download/v2.7.3/stack-2.7.3-linux-x86_64.tar.gz
Source1  : https://github.com/commercialhaskell/stack/releases/download/v2.7.3/stack-2.7.3-linux-x86_64.tar.gz.asc
Summary  : The Haskell Tool Stack
Group    : Development/Tools
License  : BSD-3-Clause
Requires: stack-bin = %{version}-%{release}
Requires: stack-license = %{version}-%{release}
Patch1: 0001-Add-Makefile-with-install-target.patch

%description
Stack is a cross-platform program for developing Haskell projects. It is
intended for Haskellers both new and experienced.

%package bin
Summary: bin components for the stack package.
Group: Binaries
Requires: stack-license = %{version}-%{release}

%description bin
bin components for the stack package.


%package license
Summary: license components for the stack package.
Group: Default

%description license
license components for the stack package.


%prep
%setup -q -n stack-2.7.3-linux-x86_64
cd %{_builddir}/stack-2.7.3-linux-x86_64
%patch1 -p1

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1626884652
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=auto "
export FCFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=auto "
export FFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=auto "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=auto "
make  %{?_smp_mflags}


%install
export SOURCE_DATE_EPOCH=1626884652
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/package-licenses/stack
cp %{_builddir}/stack-2.7.3-linux-x86_64/LICENSE %{buildroot}/usr/share/package-licenses/stack/1bb0fa2552ebbc0d49251146f4f31ef0567ad345
%make_install

%files
%defattr(-,root,root,-)

%files bin
%defattr(-,root,root,-)
/usr/bin/stack

%files license
%defattr(0644,root,root,0755)
/usr/share/package-licenses/stack/1bb0fa2552ebbc0d49251146f4f31ef0567ad345
